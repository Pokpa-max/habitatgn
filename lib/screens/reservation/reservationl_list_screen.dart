...
class HouseReservationListScreen extends ConsumerStatefulWidget {
  const HouseReservationListScreen({super.key});

  @override
  ConsumerState<HouseReservationListScreen> createState() =>
      _HouseReservationListScreenState();
}

class _HouseReservationListScreenState
    extends ConsumerState<HouseReservationListScreen> {
  ...
  void initState() {
    ...
    // Délayer l'initialisation après la construction
  ...
  void _showFilterModal() {
    showModalBottomSheet(
      ...
      builder: (context) => ReservationFilterModal(
    ...
    return AppBar(
      ...
      title: Column(
        children: [
          ...
          Text(
            'Trouvez et réservez votre visite',
            style: GoogleFonts.poppins(
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Rechercher un logement à réserver...',
          ...
          children: [
            ...
            Text(
              isSearchMode
                  ...
                  : 'Il n\'y a actuellement aucun logement disponible pour réservation.',
  ...
  Widget _buildHouseList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildHouseCardWithReservation(
          context,
          house,
          onReservationChanged: () {
            // Recharger la liste après une réservation
            ref.read(dashbordViewModelProvider.notifier).refreshAll();
          },
        );
      },
    );
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // CARTE DE LOGEMENT AVEC SYSTÈME DE RÉSERVATION
  // ────────────────────────────────────────────────────────────────────────────
  Widget _buildHouseCardWithReservation(
    BuildContext context,
    House house, {
    VoidCallback? onReservationChanged,
  }) {
    // Simuler les données de réservation (vous pouvez les récupérer depuis Firestore)
    final houseData = {
      'reservationStatus':
          'available', // Vous devrez récupérer cela depuis Firestore
      'reservationDetails': null,
    };

    final reservationStatus =
        _reservationService.getReservationStatus(houseData);
    final canReserve = _reservationService.canUserReserve(houseData);
    final hasUserReserved = _reservationService.hasUserReserved(houseData);
...
// ────────────────────────────────────────────────────────────────────────────
// MODAL FILTRES POUR RÉSERVATION
// ────────────────────────────────────────────────────────────────────────────
class ReservationFilterModal extends StatefulWidget {
  ...
  @override
  State<ReservationFilterModal> createState() => _ReservationFilterModalState();
}

class _ReservationFilterModalState extends State<ReservationFilterModal> {
          ...
          Flexible(
            child: SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // Header avec icône spéciale réservation
                  Row(
                    children: [
                      Container(
