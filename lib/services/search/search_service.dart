...
class SearchService {
  ...
  Future<List<House>> searchHouses({
    ...
    if (type.isNotEmpty) {
      query = query.where('houseType.label', isEqualTo: type);
    }
    if (location.isNotEmpty) {
      query = query.where('address.town.value', isEqualTo: location);
    }
    if (commune.isNotEmpty) {
      query = query.where('address.commune.value', isEqualTo: commune);
    }
    if (quartier.isNotEmpty) {
      query = query.where('address.zone', isEqualTo: quartier);
    }
    if (partNumber.isNotEmpty) {
      query = query.where('bedrooms', isEqualTo: int.parse(partNumber));
    }
    ...
    query = query
        .where('isAvailable', isEqualTo: false)
        .where('isPurchaseMode', isEqualTo: isVenteSelected);
