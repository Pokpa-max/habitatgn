...
class _ReservationModalState extends State<ReservationModal> {
  ...
  TimeOfDay? _checkInTime; // << NEW: heure d’arrivée
                      ...
                      children: [
                        ...
                        Expanded(
                          ...
                          child: ElevatedButton(
                            onPressed: (_isLoading || !isFormReady)
                                ? null
                                : _submitReservation,
  ...
  Future<void> _selectCheckInTime() async {
    if (_checkInDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Sélectionnez d\'abord la date d\'arrivée',
              style: GoogleFonts.poppins()),
          backgroundColor: Colors.orange[600],
        ),
      );
    ...
    final TimeOfDay? picked = await showTimePicker(
      context: context,
      initialTime: const TimeOfDay(hour: 14, minute: 0), // 14:00 par défaut
      builder: (context, child) {
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: ColorScheme.light(
  ...
  Future<void> _submitReservation() async {
    ...
    if (!isFormReady) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Veuillez sélectionner les dates et l’heure d’arrivée',
              style: GoogleFonts.poppins()),
          backgroundColor: Colors.orange[600],
        ),
      );
    ...
    try {
      ...
      // NOTE: adapte si ton service attend d’autres params.
